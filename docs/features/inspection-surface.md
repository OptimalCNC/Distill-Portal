...
Title-cell rendering (Phase 6 M2, Resolved Decision #7): the visible truncation is a pure CSS rule on `.title-cell-title` (`overflow: hidden; text-overflow: ellipsis; white-space: nowrap;` in `SessionsTable.css`) so the title clips at the column's available width. The full row title is always carried on the cell's native HTML `title=` attribute (`title={row.title ?? ""}` in `SessionsTable.tsx`), so the native browser tooltip + assistive technologies still expose the complete string regardless of column width. No JS character-count constant, no second display field, no popover — single source of truth is `row.title`; the truncation is presentation-only.
