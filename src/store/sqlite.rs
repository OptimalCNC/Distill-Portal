...
use crate::{
    collect::ScanErrorRecord,
    ingest::{ParsedSession, Tool},
    store::{blob_store::StoreError, migrations},
};
...
impl SqliteStore {
    ...
    pub fn get_session(&self, session_uid: &str) -> Result<Option<SessionRecord>, StoreError> {
        let connection = self.connection()?;
        query_session(
            &connection,
            "SELECT session_uid, tool, source_session_id, source_path, source_fingerprint, raw_ref,
    ...
    pub fn record_scan_error(&self, error: &ScanErrorRecord) -> Result<(), StoreError> {
        let connection = self.connection()?;
        let now = now_rfc3339();
    ...
    pub fn insert_session(
        ...
        transaction.execute(
            ...
            params![
                ...
                parsed.source_path.display().to_string(),
                parsed.source_fingerprint,
                raw_ref,
                format_optional_time(parsed.created_at),
                format_optional_time(parsed.source_updated_at),
                format_time(ingested_at),
                parsed.project_path.as_ref().map(|path| path.display().to_string()),
                parsed.title,
                parsed.has_subagent_sidecars as i64,
            ],
        )?;
        transaction.commit()?;
        drop(connection);
...
fn tool_from_db(value: &str) -> Result<Tool, rusqlite::Error> {
    match value {
        "claude_code" => Ok(Tool::ClaudeCode),
        "codex" => Ok(Tool::Codex),
...
fn scan_error_id(tool: Tool, source_path: &Path) -> String {
    crate::ingest::sha256_hex(format!("{}:{}", tool.as_str(), source_path.display()).as_bytes())
}
